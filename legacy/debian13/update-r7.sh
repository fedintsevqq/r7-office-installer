...
# ============================================================
#  🚀 Р7 Офис Установщик - ПОЛНАЯ ВЕРСИЯ
#  Версия: 5.2 - MD5 (ручной ввод), GUI, справка, автоустановка zenity
# ============================================================
...
header() {
    clear
    separator
    echo -e "${BOLD}${WHITE}  🚀  Р7 ОФИС - ПРОДВИНУТЫЙ УСТАНОВЩИК  ${NC}"
    echo -e "${BOLD}${WHITE}  🔧  MD5 (ручной ввод), GUI, СПРАВКА  ${NC}"
    separator
    echo ""
}
...
check_root() {
    if [[ $EUID -ne 0 ]]; then
        ...
        echo -e "${YELLOW}   Используйте: sudo update-r7.sh${NC}"
        exit 1
    fi
}
...
show_help() {
    cat << EOF
Использование: update-r7.sh [ОПЦИИ]

ОПЦИИ:
  -v, --version VERSION   Установить конкретную версию (например, 2815)
  ...
  --check                 Проверить контрольную сумму (MD5 / SHA256)
  --md5 HASH              Задать MD5-хэш для проверки (если нет .md5 файла)
  --gui                   Использовать графический интерфейс (zenity)
  -h, --help              Показать эту справку

Примеры:
  update-r7.sh -v 2815 --md5 a1b2c3...          # установить с проверкой MD5
  update-r7.sh -l --check                       # проверить по .md5 файлу
  update-r7.sh -u https://.../r7.deb --md5 abc # скачать и проверить
  update-r7.sh --gui                            # графический режим
  update-r7.sh -h                               # справка
EOF
}
...
check_checksum() {
    ...
    if [ -n "$user_md5" ]; then
        echo -e "${BLUE}🔍 Проверка MD5 (передан через аргумент)...${NC}"
    ...
    if [ -f "$sha_file" ]; then
        echo -e "${BLUE}🔍 Проверка SHA256...${NC}"
        local computed=$(sha256sum "$file" | awk '{print $1}')
        local expected=$(cat "$sha_file" | awk '{print $1}')
        ...
        echo -e "${YELLOW}⚠️  Файлы .sha256 или .md5 не найдены.${NC}"
        echo -e "${YELLOW}   Если у вас есть MD5-хэш из документа Word, введите его.${NC}"
        echo -n -e "${BOLD}Введите MD5 (или нажмите Enter для пропуска): ${NC}"
        read -r manual_md5
...
detect_environment() {
    ...
    if [ -f /etc/debian_version ]; then
        [ "$CMD_GUI" = false ] && echo -e "  ${GREEN}✅${NC} Debian: $(cat /etc/debian_version)"
    fi
    ...
    if [ "$FREE_SPACE" -lt 5 ]; then
        if [ "$CMD_GUI" = true ]; then
            ...
            echo -e "  ${RED}❌${NC} Мало места: ${FREE_SPACE}G"
        fi
        return 1
    else
...
install_all_dependencies() {
    ...
    declare -A DEPENDENCIES=(
        ...
        ["libgconf-2-4"]="libgconf-2-4_3.2.6-8"
        ["gconf2-common"]="gconf2-common_3.2.6-8"
    ...
    [ "$CMD_GUI" = false ] && echo -e "${YELLOW}📦 Будет установлено ${#deps_to_install[@]} пакетов${NC}"

    if [[ " ${deps_to_install[*]} " =~ " libgconf-2-4 " ]] || [[ " ${deps_to_install[*]} " =~ " gconf2-common " ]]; then
        [ "$CMD_GUI" = false ] && echo -e "${BLUE}→ Установка libgconf-2-4 (из Debian 12)...${NC}"
        cd "$DOWNLOAD_DIR" || exit 1
        if [ ! -f "libgconf-2-4_3.2.6-8_amd64.deb" ]; then
            wget -q --show-progress https://ftp.debian.org/debian/pool/main/g/gconf/libgconf-2-4_3.2.6-8_amd64.deb
        fi
        if [ ! -f "gconf2-common_3.2.6-8_all.deb" ]; then
            wget -q --show-progress https://ftp.debian.org/debian/pool/main/g/gconf/gconf2-common_3.2.6-8_all.deb
        fi
        dpkg -i gconf2-common_3.2.6-8_all.deb 2>&1 | tee -a "$LOG_FILE"
        dpkg -i libgconf-2-4_3.2.6-8_amd64.deb 2>&1 | tee -a "$LOG_FILE"
        deps_to_install=("${deps_to_install[@]/libgconf-2-4/}")
        deps_to_install=("${deps_to_install[@]/gconf2-common/}")
    fi
    ...
    if [ ${#deps_to_install[@]} -gt 0 ]; then
        ...
        if [ ${#filtered_deps[@]} -gt 0 ]; then
            [ "$CMD_GUI" = false ] && echo -e "${BLUE}→ Установка остальных через APT...${NC}"
            apt install -y "${filtered_deps[@]}" 2>&1 | tee -a "$LOG_FILE"
        fi
    fi

    [ "$CMD_GUI" = false ] && echo -e "${GREEN}✅ ВСЕ ЗАВИСИМОСТИ УСТАНОВЛЕНЫ!${NC}"
    touch "$DEPENDENCIES_FLAG"
    return 0
}
...
remove_r7_office() {
    ...
    read -r choice
    case "$choice" in
        1) apt remove -y r7-office; apt autoremove -y; echo -e "${GREEN}✅ Приложение удалено.${NC}" ;;
        2) apt remove -y r7-office; apt autoremove -y; CACHE_DIR="$USER_HOME/.local/share/r7-office/editors/recover"; [ -d "$CACHE_DIR" ] && rm -rf "$CACHE_DIR"/*; echo -e "${GREEN}✅ Приложение удалено, кэш очищен.${NC}" ;;
        3) echo -n -e "${BOLD}Подтвердите полное удаление (введите YES): ${NC}"; read -r confirm; if [ "$confirm" != "YES" ]; then echo -e "${YELLOW}❌ Отмена${NC}"; return 0; fi; apt remove -y r7-office; apt autoremove -y; rm -rf "$USER_HOME/.local/share/r7-office"; rm -rf "$USER_HOME/.config/r7-office"; rm -f "$DEPENDENCIES_FLAG"; echo -e "${GREEN}✅ Полное удаление завершено.${NC}" ;;
        4|*) echo -e "${YELLOW}❌ Отмена${NC}" ;;
    esac
    echo -n -e "${YELLOW}Нажмите Enter для продолжения...${NC}"
    read -r
}
